...
if [ "$DOCKER_IMAGE_HASH" == "null" ]; then
  ...
  DOCKER_IMAGE_HASH="$(docker build --platform linux/amd64 -q -t $SANDBOX_NAME -f $DOCKER_FILE $ARGS .)"
...
docker run --platform linux/amd64 --privileged=true -u 0 -it $PORTS $ENVS -v /var/run/docker.sock:/var/run/docker.sock $MOUNT -w $WORK_DIR --name $SANDBOX_NAME -d $DOCKER_IMAGE_HASH
